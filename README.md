...
I chose to compare an optimal return portfolio against an equally weighted portfolio of the same stocks. This increases the probability that my portfolio will outperform a basic strategy. By visualizing both portfolios on a monthly cumulative basis we see the optimal weight portfolio has a higher return.
...
Next I run a Fama French 4 factor simulation on two portfolios, equal weight and optimal weight to determine which portfolio strategy was more effective under various conditions.
...
**Conclusion**
Although the trading strategies employed in this analysis were simple, it highlights the FF 4-factor model's efficacy in evaluating trading strategies. To further this analysis it would be useful to build models for other trading strategies. Additionally, selecting stocks using a non-random method would result in a more broad range of real world applications.
